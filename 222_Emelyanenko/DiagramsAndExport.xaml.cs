...
{
    /// <summary>
    /// Логика взаимодействия для DiagramsAndExport.xaml
    /// </summary>
    public partial class DiagramsAndExport : Page
    {
        public DiagramsAndExport()
        {
            InitializeComponent();
            try
            {
                CmbUser.ItemsSource = Emelyanenko_DB_PaymentEntities2.getInstance().User.ToList();
            }
            catch (Exception ex)
            {
            ...
            ChartPayments.Series.Add(new Series("Платежи"));
        }
        ...
        private void UpdateChart(object sender, SelectionChangedEventArgs e)
        {
            if (CmbUser.SelectedItem is User currentUser && CmbDiagram.SelectedItem is SeriesChartType currentType)
            {
                ...
                HashSet <Category> categoriesList = Emelyanenko_DB_PaymentEntities2.getInstance().Payment.ToList().Where(payment => payment.User == currentUser).Select(payment => payment.Category).ToHashSet();
                foreach (Category category in categoriesList)
                {
                    currentSeries.Points.AddXY(category.Name, Emelyanenko_DB_PaymentEntities2.getInstance().Payment.ToList().Where(payment => payment.User == currentUser && payment.Category == category).Sum(payment => payment.Price * payment.Num));
                }
            }
        }
        ...
        {
            Emelyanenko_DB_PaymentEntities2 _context = Emelyanenko_DB_PaymentEntities2.getInstance();
            ...
            foreach (var user in allUsers)
            {
                ...
                for (int i = 0; i < allCategories.Count(); i++)
                {
                    ...
                    Where(u => u.Category == currentCategory).Sum(u => u.Num *
                    u.Price).ToString("N2") + " руб.";
                    cellRange.Font.Name = "Times New Roman";
            ...
            foreach (Microsoft.Office.Interop.Word.Section section in document.Sections)
            {
                ...
                Microsoft.Office.Interop.Word.WdParagraphAlignment.wdAlignParagraphCenter; headerRange.Font.ColorIndex = Microsoft.Office.Interop.Word.WdColorIndex.wdBlack;
                headerRange.Font.Size = 10; headerRange.Text =
                DateTime.Now.ToString("dd/MM/yyyy");
            }
            try
            {
                document.SaveAs2(Environment.CurrentDirectory + "/Payments.docx");
                document.SaveAs2(Environment.CurrentDirectory + "/Payments.pdf", Word.WdExportFormat.wdExportFormatPDF);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        private void ExcelExport(object sender, RoutedEventArgs e)
        {
            ...
            summarySheet.Cells[1, 2] = Emelyanenko_DB_PaymentEntities2.getInstance().Payment.Select(entry => entry.Price * entry.Num).Sum();
            //Форматирование: красный цвет и жирный шрифт

            Excel.Range summaryRange = summarySheet.Range[summarySheet.Cells[1,
            1], summarySheet.Cells[1, 2]];
